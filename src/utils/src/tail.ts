class TailElement {
 ...
 export default class RenderTail {
    constructor(colors, characters,drawType) {
      this.drawType = drawType||1; //类型
      this.showTime = 20;
      this.maxWidth = 20;
      this.minWidth = 5;
      this.color = "";
      ...
      switch (this.drawType) {
        case 1:
          const render1 = () => {
            ...
            let loop = 0;
            let line = 0;
            let lineMax = this.maxWidth;
            let lineMin = this.minWidth;
            let TWO_PI = 2 * Math.PI;
            let t = 0;
            this.animate = true;
            let op = 1;
            let bodyLength = this.showTime;
            ...
            this.onMouseMove = mouse_track
            this.onMouseLeave = mouse_leave
            ...
            let startY = canvas.height - (canvas.height / 2 + (size * (13 * Math.cos(0) - 5 * Math.cos(0) - 2 * Math.cos(0) - Math.cos(0))));
